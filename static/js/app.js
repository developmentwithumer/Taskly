...
var Toast = {
  show(message, options = {}) {
    ...
    toast.className = "flex items-center gap-4 px-6 py-4 bg-slate-800/90 border border-slate-700/50 " + "backdrop-blur-md rounded-2xl shadow-2xl shadow-cyan-900/20 text-sm font-bold " + "opacity-0 translate-y-4 pointer-events-auto";
    toast.innerHTML = `<span>${message}</span>${options.taskId ? `<button onclick="window.undoDelete(${options.taskId})" class="ml-auto text-cyan-400 hover:text-cyan-300 hover:underline">Undo</button>` : ""}`;
    container.appendChild(toast);
    gsapWithCSS.to(toast, { y: 0, opacity: 1, duration: 0.4, ease: "power3.out" });
    setTimeout(() => {
      gsapWithCSS.to(toast, { y: -10, opacity: 0, duration: 0.3, onComplete: () => toast.remove() });
    }, 5000);
  }
};
function attachUndoHandler(renderCallback) {
  window.undoDelete = (taskId) => {
...
// src/js/api.ts
var API_URL2 = "/api/tasks";
var tasks = [];
function updateTasksArray(newTasks) {
  tasks = newTasks;
}
async function fetchTasks(renderCallback) {
  try {
...
async function saveTask(payload, id) {
  const isUpdate = !!id;
  const url = isUpdate ? `${API_URL2}/${id}` : API_URL2;
...
async function deleteTask(id, renderCallback) {
  ...
  pendingDeletions[id] = {
    ...
    timeout: setTimeout(async () => {
      try {
        await fetch(`${API_URL2}/${id}`, { method: "DELETE" });
...
// src/js/ui.ts
var formatDateForInput = (isoDate) => {
  if (!isoDate)
    return "";
  return new Date(isoDate).toISOString().slice(0, 16);
};
var toLocalInputValue = (date) => {
  const pad = (n) => n.toString().padStart(2, "0");
  return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`;
};
function setupDragAndDrop(renderCallback) {
...
function openModal(task) {
  ...
  if (modeLabel)
    modeLabel.innerText = task ? "Edit Task" : "New Task";
  document.getElementById("task-id").value = task ? task.id.toString() : "";
  ...
  document.getElementById("task-start").value = task ? formatDateForInput(task.start_date) : toLocalInputValue(now);
  document.getElementById("task-due").value = task ? formatDateForInput(task.due_date) : toLocalInputValue(tmr);
  const startInput = document.getElementById("task-start");
  const dueInput = document.getElementById("task-due");
  const useNowCb = document.getElementById("use-current-date");
  const use24hCb = document.getElementById("use-24h-due");
  useNowCb.checked = false;
  use24hCb.checked = false;
  const newUseNow = useNowCb.cloneNode(true);
  useNowCb.parentNode?.replaceChild(newUseNow, useNowCb);
  newUseNow.addEventListener("change", () => {
    if (newUseNow.checked)
      startInput.value = toLocalInputValue(new Date);
  });
  const newUse24h = use24hCb.cloneNode(true);
  ...
  gsapWithCSS.to(modal, { opacity: 1, duration: 0.3 });
  gsapWithCSS.to(backdrop, { opacity: 1, duration: 0.4 });
  gsapWithCSS.to(content, { y: 0, scale: 1, opacity: 1, duration: 0.5, ease: "back.out(1.7)" });
  gsapWithCSS.to("#kanban-board", { opacity: 0.3, filter: "blur(4px)", duration: 0.4 });
}
function closeModal() {
  const modal = document.getElementById("focus-modal");
  const content = document.getElementById("task-form");
  if (!modal || !content)
    return;
  gsapWithCSS.to("#kanban-board", { opacity: 1, filter: "blur(0px)", duration: 0.4 });
  gsapWithCSS.to(content, { y: 20, scale: 0.95, opacity: 0, duration: 0.3 });
  gsapWithCSS.to(modal, { opacity: 0, duration: 0.4, delay: 0.1, onComplete: () => modal.classList.add("pointer-events-none") });
}
var currentViewingTask = null;
...
function openProfileModal() {
  ...
  gsapWithCSS.to(modal, { opacity: 1, duration: 0.3 });
  gsapWithCSS.to(backdrop, { opacity: 1, duration: 0.4 });
  gsapWithCSS.to(form, { y: 0, opacity: 1, duration: 0.5, ease: "back.out(1.7)" });
  gsapWithCSS.to("#kanban-board, aside, header", { opacity: 0.3, filter: "blur(4px)", duration: 0.4 });
}
function closeProfileModal() {
  ...
  if (!modal || !form)
    return;
  gsapWithCSS.to("#kanban-board, aside, header", { opacity: 1, filter: "blur(0px)", duration: 0.4 });
  gsapWithCSS.to(form, { y: 20, opacity: 0, duration: 0.3 });
  gsapWithCSS.to(modal, { opacity: 0, duration: 0.4, delay: 0.1, onComplete: () => modal.classList.add("pointer-events-none") });
}
...
var timeRemaining = FOCUS_DURATION;
var isRunning = false;
var _renderBoard = null;
function initFocusTimer(renderBoardCb) {
...
function openFocusModal(title) {
  ...
  if (!modal || !content)
    return;
  modal.classList.remove("pointer-events-none");
  gsapWithCSS.to(modal, { opacity: 1, duration: 0.7, ease: "power2.inOut" });
  gsapWithCSS.to(content, { y: 0, duration: 0.8, delay: 0.1, ease: "back.out(1.5)" });
}
function stopAndCloseFocus() {
  ...
  if (!modal)
    return;
  gsapWithCSS.to(content, { y: 30, duration: 0.5, ease: "power2.in" });
  gsapWithCSS.to(modal, { opacity: 0, duration: 0.6, delay: 0.1, ease: "power2.inOut", onComplete: () => modal.classList.add("pointer-events-none") });
}
  ...
  if (isRunning) {
    ...
    timerInterval = setInterval(() => {
      if (timeRemaining > 0) {
        ...
        if (display)
          gsapWithCSS.fromTo(display, { scale: 1.1, color: "#4ade80" }, { scale: 1, color: "white", duration: 1 });
        playIcon?.classList.remove("hidden");
        pauseIcon?.classList.add("hidden");
        if (currentViewingTask) {
          currentViewingTask.status = "done";
...
// src/js/main.ts
var socket = io();
function toLocalInputValue2(date) {
  const pad = (n) => n.toString().padStart(2, "0");
  return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}`;
}
...
function resolveDisplayColumn(task) {
  if (task.status === "done")
    return "done";
  if (isOverdue(task))
    return "overdue";
...
function renderBoard() {
  ...
  if (!cols.todo || !cols.in_progress || !cols.done || !cols.overdue)
    return;
  cols.todo.innerHTML = "";
  ...
  const visibleTasks = tasks.filter((task) => {
    ...
    return `${task.title} ${task.description || ""} ${task.status.replace("_", " ")} ${task.priority} ${sd} ${dd}`.toLowerCase().includes(searchQuery);
  });
  const counts = { todo: 0, in_progress: 0, done: 0, overdue: 0 };
  visibleTasks.forEach((task) => {
    const colKey = resolveDisplayColumn(task);
    counts[colKey]++;
...
var tasksNavOpen = false;
window.toggleTasksNav = () => {
  ...
  if (sub)
    sub.style.maxHeight = tasksNavOpen ? "300px" : "0";
  if (chevron)
    chevron.style.transform = tasksNavOpen ? "rotate(180deg)" : "";
};
window.filterBoard = (filter) => {
  ...
  ["todo", "in_progress", "done", "overdue"].forEach((status) => {
    const col = document.getElementById(`drop-${status}`);
    if (col)
      col.style.display = filter === "all" || filter === status ? "" : "none";
  });
};
window.delTask = (id) => deleteTask(id, renderBoard);
window.delTaskFromView = () => {
  if (currentViewingTask) {
    window.delTask(currentViewingTask.id);
    closeViewModal();
  }
};
socket.on("task_created", (task) => {
  updateTasksArray([...tasks, task]);
  renderBoard();
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("task-form")?.addEventListener("submit", (e) => {
    ...
    const dueInput = document.getElementById("task-due");
    const now = new Date;
    if (document.getElementById("use-current-date")?.checked)
      startInput.value = toLocalInputValue2(now);
    if (document.getElementById("use-24h-due")?.checked)
      dueInput.value = toLocalInputValue2(new Date(now.getTime() + 24 * 60 * 60 * 1000));
    ...
    if (status === "todo" && startDt && isToday(startDt.toISOString()))
      status = "in_progress";
    saveTask({
  ...
  document.getElementById("edit-profile-action")?.addEventListener("click", (e) => {
    e.preventDefault();
    openProfileModal();
  });
  ...
  gsapWithCSS.from("aside", { x: -50, opacity: 0, duration: 0.8, ease: "power3.out" });
  gsapWithCSS.from("header", { y: -30, opacity: 0, duration: 0.8, delay: 0.2, ease: "power3.out" });
  gsapWithCSS.from("#kanban-board > div", { y: 30, opacity: 0, duration: 0.6, delay: 0.4, stagger: 0.15, ease: "power2.out" });
